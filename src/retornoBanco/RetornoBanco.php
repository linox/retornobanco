...
class RetornoBanco {
	...
	*/
    ...
    public function lerArquivo($nome)
    {
		...
		fclose($lendo);
		echo '<hr/>';
		echo 'Valor pago = '.preg_replace('/^0+/','',$u_v_pago);
		echo '<br>Pagamento = '.$u_dt_ocorencia;
		echo '<br>Status = Pago';
		echo '<br>Nosso nº '.$t_id_titulo_banco;
		echo '<br>Gerado em '.$data_agora.' as '.$hora_agora;
		echo '<br>Retorno = '.$nome;
